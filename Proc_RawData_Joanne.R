...
setwd('C:/Users/PinWei/my_CSR/Data_SRT_Joanne')
...
for ( inst_type in c("explicit", "implicit") ) {
  
  file_paths <- list.files(
    path = file.path("RawData", inst_type), 
    pattern = ".csv$", 
    recursive = TRUE, 
    full.names = TRUE
  )
  ...
  for ( fp in file_paths ) {
    ...
    if ( sid %in% learned_subjs ) {
      subj_group <- "learned"
    } else {
      subj_group <- "not_learned"
    }
    
    out.dir <- file.path(
      "IntermediateData", inst_type, subj_group)
    ...
    write.csv(
      DF, row.names = FALSE, 
      file = file.path(
        out.dir, sprintf('sub_%02d.csv', sid)))
    ...
    write.csv(
      ...
      file = file.path(
        out.dir, sprintf('sub_%02d_cleaned.csv', sid)))
  }
}
